"""
Official implementation for AquaFeat-YOLOv10.

This script integrates the AquaFeat enhancement module into the Ultralytics
YOLOv10s backbone for improved underwater object detection. It includes:
...
2.  Logic to dynamically build a new YOLOv10s model YAML.
...
Example Usage:
    python AquaFeatYoloV10.py \
        --weights /path/to/yolov10s.pt \
        --data /path/to/your_dataset.yaml \
        --project yolov10_aquafeat_runs \
        --name aquafeat_yolov10s_exp1 \
        --epochs 150 \
        --batch 6 \
        --imgsz 608
"""
...
class ColorCorrectionBlock(nn.Module):
    ...
    """
    def __init__(self):
        """Initializes the deterministic color correction block."""
        super().__init__()
...
class AquaFeat(nn.Module):
    ...
    This module is designed to be prepended to a detection backbone (like YOLOv10)
...
# as the very first layer (layer 0). This definition is for YOLOv10s.
MODEL_YAML_TEMPLATE = """
...
def main(args):
    ...
    try:
        ...
        logging.error("Please ensure '--weights' points to a valid YOLOv10 .pt file.")
        return

    # --- 3. Create Custom Model YAML ---
    final_yaml_content = MODEL_YAML_TEMPLATE.format(nc=nc)
    model_yaml_filename = f"yolov10_{args.name}_temp.yaml"
    ...
    for k, v in og_state_dict.items():
        ...
        # model.0. ... -> model.1. ...
        # ...
        # model.21. ... -> Detect head, no shift
        if key_parts[0] == 'model' and key_parts[1].isdigit():
            original_layer_idx = int(key_parts[1])
            # The 'Detect' layer index (21 in v10s) and beyond are not shifted
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train YOLOv10s with AquaFeat Enhancement Module"
    )
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True, 
        help="Path to the pre-trained YOLOv10 .pt file (e.g., yolov10s.pt)"
    )
    parser.add_argument(
        '--data', 
        type=str, 
    ...
    parser.add_argument(
        ...
        type=int, 
        default=608, 
        help="Image size for training (e.g., 608, 640)"
    )
    parser.add_argument(
        '--optimizer', 
        type=str, 
    ...
    parser.add_argument(
        '--project', 
        type=str, 
        default='runs/train_aquafeat_v10', 
